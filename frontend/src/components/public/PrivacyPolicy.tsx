...
import { Shield, Edit2, X, Save, Loader2 } from 'lucide-react';
...
const PrivacyPolicy: React.FC = () => {
  ...
  const fetchDoc = async () => {
    try {
      const res = await fetch(`${API_URL}/docs/privacy-policy`);
      if (res.ok) {
      ...
      toast.error(language === 'vi' ? 'Không thể lấy nội dung chính sách bảo mật' : 'Cannot retrieve privacy policy content');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(language === 'vi' ? 'Lưu chính sách bảo mật thành công!' : 'Privacy policy saved successfully!');
        setIsEditing(false);
      } else {
        const data = await res.json();
  ...
  // Custom components for ReactMarkdown to render React Router Link for internal links
  ...
  const displayContent = (language === 'en' && (!content || content.trim().startsWith('# Chính sách quyền riêng tư')))
    ? defaultDocsEn['privacy-policy']
    : content;
            ...
            {isEditing ? (
              <div className="flex-1 flex flex-col gap-4 no-prose">
                <div className="flex items-center gap-2 mb-1">
                  <Shield className="w-5 h-5 text-[#2c4a3e]" />
